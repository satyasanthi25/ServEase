...
class User(db.Model, UserMixin):
    ...
    #service = db.relationship('Service', backref='users', lazy=True)
    ...
    provided_services = db.relationship('ServiceRequest', foreign_keys='ServiceRequest.professional_id', back_populates='professional')
...
class ServiceRequest(db.Model):
    ...
    service_request_status = db.Column(db.String(20), default='requested')#accepted,closed,pending,requested,rejected
    ...
    professional = db.relationship('User', foreign_keys=[professional_id], back_populates='provided_services')
    @property
    def assigned_professional(self):
        rqs = ServiceRequest.query.filter_by(
            service_id=self.service_id, is_accepted=True, is_requested=False, is_closed=False
        ).first()
        return rqs.professional if rqs else None
